...
def visualize_image_attention(image_id, boxes, att_weights):
    im = cv2.imread(
        "/nfs/users/zhouzixuan/Cross-Attention-VizWiz-VQA-main/demo/static/img/{}.png".format(image_id)
    )
    print(boxes)
    ...
    plt.imsave(
        "/nfs/users/zhouzixuan/Cross-Attention-VizWiz-VQA-main/demo/static/attention_maps/{}.png".format(
            image_id
        ),
        im_ocr_att,
    )
...
def visualize_question_attention(question, image_id, q_att):
    ...
    with open(
        "/nfs/users/zhouzixuan/Cross-Attention-VizWiz-VQA-main/demo/static/question_maps/{}.json".format(
            image_id
        ),
        "w",
    ) as outfile:
